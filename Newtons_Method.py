...
def f_double_prime(x):
	return (f_prime(x+VERY_SMALL_VALUE)-f_prime(x))/VERY_SMALL_VALUE
...
def newtons_method():
	...
	while loop_condition_is_met(x_old, x_new, precision):
		x_old = x_new
		x_new = x_old - alpha * (f_prime(x_old)/f_double_prime(x_old))
	# print the local maximum
	print "Newton's Method: " + str((round(x_new, 3), round(f(x_new), 3)))
